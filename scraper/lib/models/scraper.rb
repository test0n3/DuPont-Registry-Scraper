...
class Scraper
  ...
  def scrape
    ...
    puts "Parsed_page is #{cars.class}"
    # counts the number of cars on each page, should be 10
    per_page = cars.count
    # total_listings recovers total amount of results
    total_listings = parsed_page
                     .css('#mainContentPlaceholder_vehicleCountWithin')
                     .text.to_i
    # returns number of pages the search results are distributed
  ...
  def create_car_hash(car_obj)
    # puts "car_obj content: #{car_obj}"
    puts "what is car_obj: #{car_obj.class}"
    car_obj.map do |car|
      {
        year: car.css('a')
                 .children[0]
